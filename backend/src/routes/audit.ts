...
import { lokiQueryRange, LokiStream } from "../lib/lokiClient";
...
interface OverviewResponse {
  ...
  topUsers:         { service: string; userId: string; count: number; maskedAccess: number }[];
  maskedDataAccess: { userId: string; service: string; count: number }[];
  externalIPs:      { ip: string; userId: string; page: string; timestamp: string }[];
  suspiciousUsers:  { userId: string; service: string; count: number; uniquePages: number }[];
  recentEvents:     { timestamp: string; service: string; userId: string; ip: string; page: string; masked: boolean }[];
}
...
router.get("/overview", async (_req: Request, res: Response): Promise<void> => {
  ...
  // Flatten all events, keeping per-service association
  ...
  const pageMap = new Map<string, number>(); // key: "service||page"
  for (const ev of allEvents) {
    if (!ev.page) continue;
  ...
  // --- topUsers (top 20 overall, with maskedAccess) ---
  ...
  // --- suspiciousUsers (>100 events in 24h, skip unknown userId) ---
  ...
  // --- recentEvents (last 30, sorted desc) ---
